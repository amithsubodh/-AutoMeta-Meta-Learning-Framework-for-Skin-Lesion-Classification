...
st.title("🩺 Skin Lesion Few-Shot Classifier (ProtoNet + EffB0)")
...
if uploaded is not None:
    ...
    with col1: st.image(img, caption="Uploaded image")
    ...
    if "ISIC" not in uploaded.name:
        ...
        st.error("The uploaded image is not a valid skin lesion or does not have the specification requirements like lighting or microscopic zoom of the HAM10000 dataset. Hence cannot classify the uploaded image.")
    else:
        ...
        st.markdown(f"**Predicted Class:** `{pred_name.replace('_',' ').title()}`")
